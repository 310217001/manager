...
{
    public class ImageFile : ControllerBase
    {
        public string[] LimitPictureType = { ".PNG", ".JPG", ".JPEG", ".BMP", ".ICO" };
        /// <summary>
        /// 上传图片
        /// </summary>
        ...
        public CustomStatusCode UpLoadPhoto(IFormFile formFile, string path)
        {
            ...
            if (LimitPictureType.Contains(currentPictureExtension))
            {
                ...
                string name = DateTime.Now.ToString("yyyyMMddHHmmssfff") + currentPictureWithoutExtension + ".jpeg";
                path += name;
        ...
        /// <param name="path">路径</param>
        /// <returns></returns>
        public CustomStatusCode UpLoadFile(IFormFile formFile, string path)
        {
            CustomStatusCode code;
            var currentPictureWithoutExtension = Path.GetFileNameWithoutExtension(formFile.FileName);
            var currentPictureExtension = Path.GetExtension(formFile.FileName).ToUpper();
            ...
            string name = DateTime.Now.ToString("yyyyMMddHHmmssfff") + currentPictureWithoutExtension + currentPictureExtension;
            path += name;
            using (var fs = System.IO.File.Create(path))
            {
                formFile.CopyTo(fs);
