...
namespace Pi.PiManager.Api.Controllers.Admin.System
{
    ...
    public class ExceptionLogController : ControllerBase
    {
        ...
        public async Task<ApiResult<IEnumerable<ExceptionLogViewModel>>> List(int pageIndex, int? TypeID, string Title, DateTime? Date1, DateTime? Date2, bool? IsSolve)
        {
            ...
            var parm = Expressionable.Create<ExceptionLog>();
            parm.AndIF(!string.IsNullOrEmpty(Title), m => m.Title.Contains(Title));
            parm.AndIF(TypeID != null, m => m.Type == TypeID);
            parm.AndIF(IsSolve != null, m => m.IsSolve == IsSolve);
            parm.AndIF(Date1 != null, a => a.AddDate >= Date1);
            parm.AndIF(Date2 != null, a => a.AddDate < Date2);
        ...
        public async Task<ApiResult<object>> Delete(string ID)
        {
            ...
            if (!string.IsNullOrWhiteSpace(ID))
            {
                ...
                if (array != null && array.Length > 0)
                {
                    ...
                    res.success = db.Delete(array2) > 0 ? true : false;
                    if (res.success)
                    {
